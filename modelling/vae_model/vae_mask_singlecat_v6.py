### Masked randomly within the fixed feature
...
class FixedMaskingLayer(tf.keras.layers.Layer):
    def __init__(self, mask_index, mask_ratio=0.5, **kwargs):
        super(FixedMaskingLayer, self).__init__(**kwargs)
        self.mask_index = mask_index  # subset to consider for masking
        self.mask_ratio = mask_ratio  # proportion of data points to mask

    def call(self, inputs, training=None):
        # Shape of the input
        batch_size = tf.shape(inputs)[0]

        # Create random mask indices based on the mask_ratio
        mask_length = [4000, 4000, 4000, 1]  # lengths of DNAm, K9, K27, expression
        total_to_mask = int(
            mask_length[self.mask_index] * self.mask_ratio
        )  # Number of points to mask (get corresponding feature length and multiply by ratio)

        # Generate random indices to be masked
        random_indices = tf.random.shuffle(tf.range(mask_length[self.mask_index]))[
            :total_to_mask
        ]

        # Create the mask for the selected region (only mask random indices)
        mask_base = tf.ones(
            [batch_size, mask_length[self.mask_index]], dtype=tf.float32
        )  # create base mask of only 1s
        mask_base = tf.tensor_scatter_nd_update(
            mask_base,
            tf.expand_dims(random_indices, axis=1),
            tf.fill(
                [total_to_mask], -1.0
            ),  # update base mask to include -1 corresponding to randomly selected indices
        )
        ...
        masks = [
            tf.concat(
                [mask_base, tf.zeros([batch_size, 8001], dtype=tf.float32)], axis=1
            ),  # DNAm
            tf.concat(
                [
            ...
            tf.concat(
                [tf.zeros([batch_size, 12000], dtype=tf.float32), mask_base], axis=1
            ),  # expression
        ]
...
plt.suptitle(
    "VAE Training and Validation Losses (mask (0.5) = {})".format(section_name)
)
plt.tight_layout(rect=[0, 0, 1, 0.95])

plt.savefig("vae_loss_plot_{}_100_0.5_v6.png".format(section_name))
...
plt.suptitle(
    "VAE Training and Validation Losses (mask (0.5) = {}) - Log Scale".format(
        section_name
    )
)
plt.tight_layout(rect=[0, 0, 1, 0.95])

plt.savefig("vae_loss_plot_{}_100_0.5_v6_log.png".format(section_name))

### Checking that KL Loss is being tracked properly (with KLDivergence metric)
fig, axs = plt.subplots(1, 2, figsize=(14, 6))
# Plot training and validation loss (Total Loss)
axs[0].plot(epochs, history_dict["kl_loss"], label="Train KL Loss", color="green")
axs[0].plot(
    epochs, history_dict["val_kl_loss"], label="Validation KL Loss", color="red"
)
axs[0].set_xlabel("Epochs")
axs[0].set_ylabel("KL Loss")
axs[0].legend()
axs[0].set_title("KL Loss (custom)")
...
plt.tight_layout()
plt.savefig("vae_loss_plot_{}_100_0.5_v6_KL_check.png".format(section_name))
plt.show()
