...
X_train, X_val = train_test_split(X_balanced, test_size=0.2, random_state=42)
...
plt.suptitle(
    "VAE Training and Validation Losses (Balanced & mask = {})".format(section_name)
)
plt.tight_layout(rect=[0, 0, 1, 0.95])

plt.savefig("vae_loss_plot_balanced_{}_300_v4.png".format(section_name))
...
plt.suptitle(
    "VAE Training and Validation Losses (Balanced & mask = {}) - Log Scale".format(
        section_name
    )
)
plt.tight_layout(rect=[0, 0, 1, 0.95])

plt.savefig("vae_loss_plot_balanced_{}_300_v4_log.png".format(section_name))
