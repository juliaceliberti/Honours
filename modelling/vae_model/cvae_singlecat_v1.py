...
class FixedMaskingLayer(
    ...
    def call(self, inputs, training=None):
        ...
        # Define sections of the input vector (for aggregation as condition)
        sections = [
            inputs[:, 0:4000],      # DNAm
            inputs[:, 4000:8000],   # K9
            inputs[:, 8000:12000],  # K27
            inputs[:, 12000:12001]  # Expression
        ]
        ...
        # Unmasked sections (condition): where mask == 0, use original inputs
        unmasked_indices = [index for index in range(4) if index != self.mask_index]
        binary_counts_list = []

        for idx in unmasked_indices:
            binary_counts_list.append(tf.reduce_sum(sections[idx], axis=1))  # Count the number of 1's

        # Concatenate all binary counts into one vector
        binary_counts = tf.stack(binary_counts_list, axis=1)

        return masked_inputs, binary_counts, mask
...
class VAE(tf.keras.Model):
    ...
    def call(self, inputs):
        # get masked and unmasked inputs
        masked_inputs, binary_counts, mask = FixedMaskingLayer(mask_index=mask_section)(inputs)
        # feed both input types to the encoder
        codings_mean, codings_log_var, codings = self.encoder([masked_inputs, binary_counts])
        # feed the codings and the condition to the decoder
        reconstruction = self.decoder([codings, binary_counts])
...
# Define the encoder
codings_size = 300
masked_inputs = tf.keras.layers.Input(shape=[12001])
binary_counts = tf.keras.layers.Input(shape=[3])

x = tf.keras.layers.Concatenate()([masked_inputs, binary_counts])
...
variational_encoder = tf.keras.Model(
    inputs=[[masked_inputs, binary_counts]], outputs=[codings_mean, codings_log_var, codings]
)

# Define the decoder
# feed in both codings size and unmasked data as condition
decoder_inputs = tf.keras.layers.Input(shape=[codings_size])
binary_counts_dec = tf.keras.layers.Input(shape=[3])

x = tf.keras.layers.Concatenate()([decoder_inputs, binary_counts_dec])
...
variational_decoder = tf.keras.Model(inputs=[decoder_inputs, binary_counts_dec], outputs=[x])
